<?php
  ...
  class Item {
    ...
    public function getFinishedOrders(PDO $db, int $idUser) {
        $sql = "SELECT Item.title, User.username, User.profile_picture FROM UserOrder 
                INNER JOIN Item ON UserOrder.idItem = Item.idItem 
                INNER JOIN User ON Item.sellerId = User.idUser 
                WHERE UserOrder.idUser = ? AND UserOrder.state = 'Entregue'";
        $stmt = $db->prepare($sql);
        $stmt->execute([$idUser]);
        $orders = array();
